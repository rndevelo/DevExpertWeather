...
import com.rndeveloper.myapplication.common.ErrorText
import com.rndeveloper.myapplication.common.LoadingAnimation
import com.rndeveloper.myapplication.common.MyTopAppBar
import com.rndeveloper.myapplication.common.Screen
import com.rndeveloper.myapplication.common.theme.DevExpertWeatherTheme
import com.rndeveloper.myapplication.home.composables.FavCitiesContent
import com.rndeveloper.myapplication.home.composables.FavouriteIconButtonContent
import com.rndeveloper.myapplication.home.composables.PermissionRequestEffect
...
fun HomeScreen(
    ...
    PermissionRequestEffect(permission = Manifest.permission.ACCESS_COARSE_LOCATION) {
        if (it) {
            ...
            when (uiState) {
                is Result.Success -> {
                    val state = (uiState as Result.Success<HomeViewModel.UiState>).data
        ...
        when (uiState) {
            ...
            is Result.Success -> {
                ...
                when (state.weatherResult) {
                    is Result.Loading -> {}
                    is Result.Success -> {
                        val weather = state.weatherResult.data.current
            ...
            is Result.Error -> {
                ErrorText(error = (uiState as Result.Error).exception)
                Log.d("UIStateError", "HomeScreen: ${(uiState as Result.Error).exception}")
            }
        }
    }
}
